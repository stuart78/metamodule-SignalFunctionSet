...
// MetaModule's newlib pulls in C macros for min/max which collide with the
// std:: templated versions used below. Force the std:: versions via #undef.
#undef min
#undef max
#include <algorithm>
...
void Dx7Note::init(const char patch[156], int midinote, int velocity) {
  ...
  for (int op = 0; op < 6; op++) {
    ...
    int peakEg = std::max<int>(std::max<int>(levels[0], levels[1]), std::max<int>(levels[2], levels[3]));
...
void Dx7Note::compute(int32_t *buf, int32_t lfo_val, int32_t lfo_delay,
  ...
  fb = std::max<int>(0, std::min<int>(8, fb));
